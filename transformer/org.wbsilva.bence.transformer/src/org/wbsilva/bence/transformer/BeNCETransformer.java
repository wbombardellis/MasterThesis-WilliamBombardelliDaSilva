...
public class BeNCETransformer {
	...
	public Optional<TransformationResult> transform (final Graph inputGraph, boolean forward){
		...
		if (!GraphgrammarUtil.isValidTripleGrammar(tripleGrammar)) {
			...
			//Check neighborhood preserving property
			if (GraphgrammarUtil.isNeighborhoodPreserving(inputGrammar)) {
				...
				logger.warn(String.format("Input grammar %s may not be neighborhood preserving. Aborting", inputGrammar.getName()));
				logger.warn("Transformation finished without success");
				return Optional.empty();
			}
		}
	}

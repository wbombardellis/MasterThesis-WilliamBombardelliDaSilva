\centering
	\begin{tikzpicture}[grammar]
		...
		%%
		\draw (1.5,0) node[t] (c1) {dd};
		\draw (1.5,-1) node[nont] (c2) {C};
		%%
		\draw (2.5,0) node[t] (t1) {db};
		\draw (2.5,-1) node[nont] (t2) {C};
	...
	\begin{tikzpicture}[grammar]
		...
		%%
		\draw (1.5,0) node[t] (c1) {tc};
		\draw (1.5,-1) node[nont] (c2) {AT};
		%%
		\draw (2.5,0) node[t, label=90:$t_{30}$] (t1) {co};
		\draw (2.5,-1) node[nont, label=90:$t_{31}$] (t2) {AT};
		%%
		\draw[morph] (c1) -- (s1);
		\draw[morph] (c1) -- (t1);
		\draw[morph] (c2) -- (s2);
		\draw[morph] (c2) -- (t2);			
		%%%%
		\draw[pipe] (3,\pipeBY) -- (3,-1);
		%%
		\node[rid] at (3.7,\ridY) {$r_4:$};
		\draw (3.5,0) node[empty] (s3) {$\emptyTG$};
	\end{tikzpicture} %
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\vskip 5pt
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

...
\begin{definition}
	Let $GG = (\Sigma, \Delta, S, P)$ be a graph grammar and $G$ a graph over $\Delta$, $G$ resolves into $H$ with the resolution partial function $s: V_G \pto V_G$, we write $G \resolv{s} H$ and call it a resolution step, if, and only if, the following holds:
	\begin{align*}
		& \forall v \in \dom s \. s(v) \notin \dom s \text{ and } \phi_G(s(v)) = \phi_G(v) \text{ and}\\
		V_H & = V_G \setminus \dom s \text{ and}\\
		E_H & = (E_G \setminus (\{(u,l,t) \st u \in \dom s, (u,l,t) \in E_G\} \\
			&\hspace{3.5em} \cup \{(t,l,u) \st u \in \dom s, (t,l,u) \in E_G\}))\\
		    & \cup \{(s(u),l,t) \st u \in \dom s, (u,l,t) \in E_G\} \\
		    & \cup \{(t,l,s(u)) \st u \in \dom s, (t,l,u) \in E_G\}
	\end{align*}
\end{definition}
...
\begin{definition}
	A derivation $D$ in a graph grammar with PAC is a sequence of $n$ derivation steps followed by $n$ resolution steps with $n > 0$, as follows:
	\begin{equation*}
		D = (G_0 \derivpac{r_0}{v_0}{W_0}{} G_1 \derivpac{r_1}{v_1}{W_1}{} \dots \derivpac{r_{n-1}}{v_{n-1}}{W_{n-1}}{} G_n^0 \resolv{s_0} G_n^1 \resolv{s_1} \dots \resolv{s_{n-1}} G_n^n)
	\end{equation*}
	with $s_i$ being a resolution total function $s_i : m_i(W_i) \to V_{G_n^i}$ and $m_i : W_i \to V_{G_n^i}$ the mapping from the PAC vertices generated on the derivation step $i$ to their correspondent vertices in $G_n^i$, for all $0 \le i < n$.
\end{definition}
...
Below, we demonstrate that the graph from Figure \ref{fig:classdiagram-g} is in $L(GG)$, by means of a derivation in $GG$.
...
\begin{remark}
	Given a graph grammar $GG$ with PAC, if the graph $g$ is in $L(GG)$, then $g$ has no PAC vertices. That is, the resolution steps remove all PAC vertices, because every resolution function $s_i$ is required to map to vertices that are not in its domain. This guarantees that the number of PAC vertices reduces at each resolution step with $s_i \neq \emptyset$.
\end{remark}
...
\begin{definition}
	A zone vertex with PAC of a garph $G$ is a triple $(\sigma, U, W)$, with $\sigma$ and $U$ being as explained in Definition \ref{def:zv} and $W \in V_G$ being the set of PAC vertices disjunct from $U$.
\end{definition}

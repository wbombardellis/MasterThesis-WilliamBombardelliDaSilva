...
\begin{tikzpicture}[grammar]
	...
	\draw[edge] (s2) to [bend left=-10] (s1) node [below=15pt, left=2pt] {$\tau$};
	...
	\draw[edge] (t1) -- (t2) node [vledgeLabel] {$\gamma$};
...
\begin{tikzpicture}[grammar]
	...
	\draw[edge] (s3) -- (s1) node [vredgeLabel] {$\psi$};
	%%
	\draw (2,0) node[t] (c1) {ac};
	\draw (2,-1) node[nont] (c2) {SA};
	...
	\draw[edge] (t3) -- (t1) node [vledgeLabel] {$\psi$};
	%%
	\draw[morph] (c1) -- (s1);
	\draw[morph] (c1) -- (t1);
	\draw[morph] (c2) -- (s2);

...
The complete transformation procedure is presented in the Algorithm \ref{alg:transform}. Thereby it is required that the TGG be neighborhood preserving (NP), what poses no problem to our procedure, once any TGG can be transformed into the neighborhood preserving normal form. A more detailed discussion of the NP normalization is performed in Chapter \ref{ch:Implementation}. Notice that this algorithm always terminates. A more elaborated discussion on its complexity is given in Section \ref{sec:eval-performance}.

...
%
\begin{abstract}
This work proposes a new graph grammar formalism, that introduces non-terminal symbols to triple graph grammars (TGG) and shows how to apply it for solving the model transformation problem. Our proposed formalism seems to suit code generation from models well, outperforms the standard TGG in the grammar size in one evaluated case and is able to express one transformation that we could not express with TGG.
...
Quality of service is a very common requirement for software projects, specially for safety-critical systems. A technique that aims to assure and enhance quality of software is the model-based development approach, which consists of the use of abstract models to specify aspects of the system under construction. The use of such models often allows for cheaper tests and verification as well as facilitates discussions about the system. 
...
\begin{proof}
	...
	First, since, by Definition \ref{def:tlanguage}, $\startTG{TGG} \tderiv{r_0}{s_0,c_0,t_0}{TGG} G^1$, that is
	\begin{align*}
	...
	In the other direction, we choose $c_0, t_0$ from the definition of $\startTG{TGG}$, with $\phi_{Z_c}(c_0) = S$ and  $\phi_{Z_t}(t_0) = S$. In this case, since, by Definition \ref{def:language},
	\begin{align*}
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[!h]
		...
		\State $GG \gets S(TGG)$ \Comment{see Definition \ref{def:source}}
		\State $\overline{D} \gets parse(GG,G)$ \Comment{use procedure in \cite{rozenberg1986boundary}}
...
We present in this paper a new triple graph grammar formalism, called NCE TGG, that is the result from mixing NCE graph grammars \cite{janssens1982graph} with TGG \cite{schurr1994specification} and that introduces for the first time, as far as we know, non-terminal symbols to TGG Furthermore, we demonstrate how NCE TGG can be used in the practice to solve the model transformation problem.

An experimental evaluation in Section \ref{sec:eval} assesses the usability of NCE TGG in comparison with standard TGG and reveals that our proposed approach has potential. In special, we could express a transformation with NCE TGG that we could not with TGG. And, from the other two transformations, NCE TGG outperformed TGG with a much smaller grammar.

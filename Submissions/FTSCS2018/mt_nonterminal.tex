...
Ina {Schieferdecker}\inst{1,2} \and Christian {Hein}\inst{1}}
%
...
\begin{example}{}
	...
	\end{tikzpicture}
	belongs to $L(\mathit{GG})$ because it contains only terminal vertices and $\startG{GG}$ \textit{derives} into it using the following derivation:
	\begin{equation*}
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[ht]
		...
		\Function{$\mathit{transform}$}{$\mathit{TGG}=(\Sigma, \Delta, S, P), \mathit{G}=(V_G,E_G,\phi_G)$}{$:\mathit{Graph}$}
		\State $\mathit{SG} \gets S(TGG)$ \Comment{see Definition \ref{def:source}}
		\State $\overline{D} \gets \mathit{parse}(\mathit{SG},G)$ \Comment{use procedure in \cite{rozenberg1986boundary}}
